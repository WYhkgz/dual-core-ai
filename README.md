...
🏗️ 架构概览
Dual-Core AI · 边云协同级联推理引擎
License: MIT Python 3.10+ PyTorch PRs Welcome

Dual-Core AI 是一个面向边缘-云端协同推理的轻量级AI推理框架，通过「边缘预处理→云端深度生成→边缘事实校验」三段式级联架构，实现算力消耗降低40%–60%、推理延迟降低30%–70%、幻觉率降至≤5% 的技术效果。
